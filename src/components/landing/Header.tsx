...
export default function Header({ currentPath = "/", onNavigate }: HeaderProps) {
  ...
  const [isScrolled, setIsScrolled] = useState(currentPath === "/profile" || currentPath === "/agenda" || currentPath === "/news");

  useEffect(() => {
    if (currentPath === "/profile" || currentPath === "/agenda" || currentPath === "/news") {
      setIsScrolled(true);
      return;
    }
    const handleScroll = () => {
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, targetId: string) => {
    e.preventDefault();
    if (targetId === "profil" || targetId === "agenda" || targetId === "berita") {
      const path = targetId === "profil" ? "/profile" : targetId === "agenda" ? "/agenda" : "/news";
      if (onNavigate) {
        onNavigate(path);
      } else {
