...
export default function Header({ currentPath = "/", onNavigate }: HeaderProps) {
  ...
  const [isScrolled, setIsScrolled] = useState(currentPath === "/profile" || currentPath === "/agenda" || currentPath === "/news" || currentPath === "/tutor");

  useEffect(() => {
    if (currentPath === "/profile" || currentPath === "/agenda" || currentPath === "/news" || currentPath === "/tutor") {
      setIsScrolled(true);
      return;
    }
    const handleScroll = () => {
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, targetId: string) => {
    e.preventDefault();
    if (targetId === "profil" || targetId === "agenda" || targetId === "berita" || targetId === "tutor") {
      const path = targetId === "profil" ? "/profile" : targetId === "agenda" ? "/agenda" : targetId === "berita" ? "/news" : "/tutor";
      if (onNavigate) {
        onNavigate(path);
      } else {
