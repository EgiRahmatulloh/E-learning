...
export default function LaporanNilaiTab({ activeTab, user }: Props) {
  ...
  const [isAngketApproved, setIsAngketApproved] = useState(false);
  const [angketScores, setAngketScores] = useState<any[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        setIsAngketApproved(setup.isAngketApproved);

        const gradesRes = await fetch(
          `/api/elearning/grades?setupId=${setupId}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
        ...
        const attRes = await fetch(
          `/api/elearning/laporan/student-attendances?setupId=${setupId}`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
        ...
        if (setup.isAngketApproved) {
          // Fetch angket tutor scores
          const angketRes = await fetch(
            `/api/elearning/angket-tutor-scores?setupId=${setupId}`,
            {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
              },
          ...
          const angketData = await angketRes.json();
          if (angketData.success) {
            setAngketScores(angketData.data);
          }
        }
      } catch (err: any) {
  ...
  const filteredAttendances = attendances.filter((s) =>
      ...
      <table className="w-full text-left text-sm">
        <thead className="bg-slate-50">
          <tr className="text-slate-500 font-bold text-xs uppercase tracking-wider border-b border-slate-200">
            <th className="py-4 px-6 sticky left-0 bg-slate-50 z-10 border-r border-slate-200">
              Nama Warga Belajar
            </th>
            {Array.from({ length: daysInMonth }, (_, i) => (
              <th
                key={i}
                className="py-4 px-2 text-center border-l border-slate-100 min-w-[30px]"
              >
                {i + 1}
              </th>
            ))}
            <th className="py-4 px-6 text-center border-l border-slate-200 sticky right-0 bg-slate-50 z-10">
              Rekap (%)
            </th>
          </tr>
        </thead>
          ...
          {loading ? (
            ...
            currentAttendances.map((student, i) => (
              <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                <td className="py-2 px-6 font-bold text-slate-800 sticky left-0 bg-white border-r border-slate-100">
                  {student.namaSiswa}
                </td>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-[#280f91] text-white p-6 rounded-2xl shadow-md gap-4">
        <div>
          <h3 className="text-xl font-black mb-1">
            Laporan Daftar Hadir & Nilai
          </h3>
          <p className="text-sm text-indigo-200 font-medium">
            Unduh data daftar hadir dan rekap nilai akhir warga belajar.
          </p>
        </div>
        <div className="flex flex-row flex-wrap items-center gap-2 w-full md:w-auto mt-4 md:mt-0">
          <Button
            onClick={handleExportHadir}
            className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm h-10 px-4 transition-colors shadow-lg cursor-pointer"
          >
            <FileSpreadsheet className="w-4 h-4 mr-2" /> Daftar Hadir
          </Button>
          <Button
            onClick={handleExportNilai}
            className="bg-orange-600 hover:bg-orange-500 text-white font-bold text-sm h-10 px-4 transition-colors shadow-lg cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 gap-6">
          ...
          <div className="p-6 border-b border-slate-100 flex flex-col justify-between items-start gap-4 bg-slate-50/50">
            <div className="flex flex-col md:flex-row justify-between w-full items-center gap-4">
              <div className="flex space-x-2 bg-slate-200/50 p-1 rounded-xl w-full md:w-auto">
                ...
                <Button
                  ...
                  className="flex-1 md:flex-none bg-[#280f91] hover:bg-[#1f0b73] text-white font-bold text-sm h-auto px-4 py-2.5 rounded-lg transition-colors shadow-sm cursor-pointer"
                >
            ...
            <div>
              <h3 className="text-lg font-black text-[#280f91]">
                {subTab === "kehadiran"
                  ? "Data Daftar Hadir Warga Belajar"
                  : "Data Nilai Warga Belajar"}
              </h3>
              <p className="text-xs font-semibold text-slate-500">
                {subTab === "kehadiran"
                  ? "Menampilkan rekapitulasi kehadiran warga belajar bulan ini."
                  : "Menampilkan siswa di kelas ini (Data Nilai menunggu Integrasi Fase 4)."}
              </p>
            </div>
          </div>
          ...
          {subTab === "kehadiran" ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50">
                  <tr className="text-slate-500 font-bold text-xs uppercase tracking-wider border-b border-slate-200">
                    <th className="py-4 px-6" rowSpan={2}>
                      Nama Warga Belajar
                    </th>
                    <th
                      className="py-2 px-6 text-center border-b border-slate-200"
                      colSpan={3}
                    >
                      Nilai
                    </th>
                    <th className="py-4 px-6 text-center" rowSpan={2}>
                      Nilai Akhir
                    </th>
                    <th className="py-4 px-6 text-center" rowSpan={2}>
                      Predikat
                  ...
                  <tr className="text-slate-500 font-bold text-xs uppercase tracking-wider border-b border-slate-200 bg-slate-50">
                    <th className="py-2 px-6 text-center border-l border-slate-100">
                      Tugas
                    </th>
                    <th className="py-2 px-6 text-center border-l border-slate-100">
                      Diskusi
                  ...
                  {loading ? (
                    ...
                    currentStudents.map((student) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-6 text-center font-bold text-slate-700 border-l border-slate-100">
                          {student.tugas}
                        </td>
                        <td className="py-4 px-6 text-center font-bold text-slate-700 border-l border-slate-100">
                          {student.partisipasi}
                        ...
                        <td className="py-4 px-6 text-center border-l border-slate-100">
                          <span className="text-[#ff6105] text-sm font-black">
        ...
        <Card className="p-6 border-slate-200/60 bg-slate-50 shadow-sm rounded-2xl flex flex-col h-full relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 mb-4 relative z-10">
            {!isAngketApproved && <Lock className="w-5 h-5 text-slate-500" />}
            <h3 className="text-lg font-black text-slate-700">
              Evaluasi Angket Kinerja Tutor
            </h3>
          </div>

          {isAngketApproved ? (
            <div className="space-y-4 relative z-10">
              {angketScores.length > 0 ? (
                angketScores.map((score, index) => (
                  <div
                    key={score.questionId}
                    className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"
                  >
                    <div className="flex items-start gap-3">
                      <div className="bg-slate-100 text-slate-500 font-bold w-8 h-8 rounded-lg flex items-center justify-center shrink-0">
                        {index + 1}
                      </div>
                      <p className="text-sm font-bold text-slate-700 max-w-md">
                        {score.question}
                      </p>
                    </div>
                    <div className="flex items-center gap-3 shrink-0">
                      <div className="text-right">
                        ...
                        <p className="text-lg font-black text-[#280f91]">
                          {score.avgScore.toFixed(2)} / {score.scaleMax}
                        </p>
                      </div>
                      <div className="w-12 h-12 rounded-full border-4 border-indigo-100 flex items-center justify-center font-bold text-indigo-700 bg-indigo-50">
                        {Math.round((score.avgScore / score.scaleMax) * 100)}%
                      </div>
                    </div>
                  </div>
                ))
                ...
                <div className="p-6 text-center text-slate-500 font-medium">
                  Belum ada respons angket dari siswa untuk kelas ini.
                </div>
              )}
            </div>
          ) : (
            <div className="flex-1 flex flex-col justify-center items-center text-center p-8 border-2 border-dashed border-slate-300 rounded-xl bg-white/60 backdrop-blur-sm relative z-10">
              ...
              <p className="text-slate-600 font-black mb-2 text-lg">
                Skor Evaluasi Terkunci
              </p>
              <p className="text-sm text-slate-500 font-medium max-w-sm">
                Tutor hanya dapat melihat hasil rekapitulasi angket dari warga
                belajar setelah disahkan dan dibuka (approve) oleh Admin.
              </p>
            </div>
          )}
        </Card>
