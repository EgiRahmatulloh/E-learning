    ...
    async function fetchCourse() {
      ...
      try {
        const setupRes = await fetch(
          `/api/elearning/setups?tutorId=${user?.id}`,
          {
            headers: {
        ...
        const actualProgram = kelasUpper.includes("PAKET A")
          ? "Paket A"
          : kelasUpper.includes("PAKET B")
            ? "Paket B"
            : "Paket C";
...
function SesiContent({
  ...
  const [pptFile, setPptFile] = useState<{ title: string; url: string } | null>(
    null,
  );
  const [tugasFile, setTugasFile] = useState<{
    title: string;
    url: string;
  } | null>(null);
  ...
  const [gradeEdits, setGradeEdits] = useState<
    Record<number, { grade: string | number; feedback: string }>
  >({});
  ...
  const renderReplies = (parentId: number, depth: number = 1): any => {
    const replies = forumPosts.filter((r) => r.parentId === parentId);
    if (replies.length === 0) return null;
    return replies.map((reply) => {
      const replyIsTutorSelf =
        reply.authorId === user?.id && reply.authorRole === "tutor";
      const replyDisplayName = replyIsTutorSelf
        ? "Tutor (Anda)"
        : reply.authorRole === "siswa"
          ? `Siswa (${reply.authorName || "Unknown"})`
          : reply.authorName;
      const replyAvatarLetter = replyIsTutorSelf
        ? "T"
        : (reply.authorName || "?").charAt(0).toUpperCase();
      const replyAvatarColorClass =
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start">
                <h5 className="font-bold text-slate-700 text-xs flex items-center gap-2">
                  {replyDisplayName}
                  {reply.authorRole === "tutor" && !replyIsTutorSelf && (
                    <span className="bg-[#ff6105] text-white px-1.5 py-0.5 rounded text-[9px]">
                      TUTOR
                    </span>
                  )}
                </h5>
                <div className="flex gap-1">
                  {reply.authorId === user?.id &&
                    reply.authorRole === user?.role && (
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => {
                            setEditingMessageId(reply.id);
                            setEditInputValue(reply.content);
                          }}
                ...
                <div className="mt-2 space-y-2">
                  ...
                  <div className="flex gap-2 justify-end">
                    ...
                    <Button
                      ...
                      size="sm"
                      className="bg-[#280f91] hover:bg-[#3a1bca] text-white h-7 text-xs"
                    >
                      Simpan
                    </Button>
                  </div>
  ...
  const loadAttendance = async () => {
    ...
    try {
      const res = await fetch(
        `/api/elearning/attendance?sessionId=${sessionId}`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        },
      );
  ...
  const submitReply = async (parentId?: number, textOverride?: string) => {
    const textToSubmit = textOverride || replyText;
  ...
  const executeDeleteMessage = async () => {
    ...
    try {
      const res = await fetch(`/api/elearning/forum/${confirmDeleteId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
  ...
  const handleGrade = async (
    ...
    try {
      const res = await fetch(
        `/api/elearning/submissions/${submissionId}/grade`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDownloadAllZip = async (sId: number) => {
    try {
      toast.info("Sedang menyiapkan file ZIP...");
      const res = await fetch(
        `/api/elearning/submissions/${sId}/download-zip`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        },
      );
      if (!res.ok) {
        ...
        throw new Error(d.message || "Gagal mengunduh ZIP");
      }
      const blob = await res.blob();
      const objectUrl = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = objectUrl;
      link.download = `submissions_sesi_${sessionNumber}.zip`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      toast.success("Berhasil mengunduh ZIP");
    } catch (err: any) {
      toast.error("Gagal mengunduh ZIP", { description: err.message });
    }
  };

  if (loading)
    return (
      <div className="p-6 text-slate-500 animate-pulse">Memuat sesi...</div>
    );
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 border-slate-200/60 bg-white shadow-sm rounded-2xl">
        <div className="flex justify-between items-center mb-4">
          ...
          <Button
            ...
            disabled={saving}
            className="bg-[#280f91] hover:bg-indigo-700 text-white font-bold"
          >
        ...
        <RichTextEditor
          ...
          placeholder="Tuliskan materi yang diajarkan sesi ini..."
        />
      </Card>
      ...
      <Card className="p-6 border-slate-200/60 bg-white shadow-sm rounded-2xl">
        ...
        <RichTextEditor
          ...
          onChange={setTujuanPembelajaran}
      ...
      <Card className="p-6 border-slate-200/60 bg-white shadow-sm rounded-2xl">
        ...
        <RichTextEditor
          ...
          onChange={setUraianKegiatan}
          placeholder="Tuliskan uraian kegiatan pembelajaran sesi ini..."
        />
      </Card>
        ...
        <Card className="p-6 border-slate-200/60 bg-white shadow-sm rounded-2xl flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h4 className="font-black text-[#280f91] text-lg leading-tight">
                Presensi Kehadiran Sesi {sessionNumber}
              </h4>
            </div>
            <p className="text-sm text-slate-500 mb-4">
              Daftar kehadiran warga belajar pada sesi ini. Sistem otomatis
              mencatat siswa yang telah mengklik tombol 'Hadir'.
            </p>
          </div>
          <Button
            variant="outline"
        ...
        <Card className="p-6 border-slate-200/60 bg-white shadow-sm rounded-2xl flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h4 className="font-black text-[#280f91] text-lg leading-tight">
                Forum Diskusi Sesi {sessionNumber}
              </h4>
            </div>
            <p className="text-sm text-slate-500 mb-4">
              Ruang tanya jawab interaktif antara tutor dan warga belajar
              terkait topik pembahasan sesi {sessionNumber}.
            </p>
          </div>
          <Button
            variant="outline"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="p-6 border-slate-200/60 bg-white shadow-sm rounded-2xl">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h4 className="font-black text-[#280f91] text-lg leading-tight">
              Materi Inisiasi Sesi {sessionNumber}
            </h4>
          </div>
          <p className="text-sm text-slate-500 mb-4">
            Unggah salinan presentasi atau dokumen bacaan utama.
          </p>
          <div
            onClick={() => pptInputRef.current?.click()}
            className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center hover:bg-slate-50 cursor-pointer transition-colors group"
          >
            <input
              type="file"
              className="hidden"
        ...
        <Card className="p-6 border-slate-200/60 bg-white shadow-sm rounded-2xl">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h4 className="font-black text-[#280f91] text-lg leading-tight">
              Materi Pengayaan Sesi {sessionNumber}
            </h4>
          </div>
          <p className="text-sm text-slate-500 mb-4">
            Tambahkan video referensi eksternal dari YouTube untuk melengkapi
            pemahaman.
          </p>
          <div className="space-y-4">
            ...
            <Button
              ...
              className="w-full bg-[#ff6105] hover:bg-[#e05200] text-white font-bold"
            >
              Simpan Link Video
            </Button>
          </div>
        ...
        <Card className="p-6 border-slate-200/60 bg-white shadow-sm rounded-2xl flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h4 className="font-black text-[#280f91] text-lg leading-tight">
                Latihan Mandiri Sesi {sessionNumber}
              </h4>
            </div>
            <p className="text-sm text-slate-500 mb-4">
              Soal pilihan ganda singkat (Quiz) untuk mengevaluasi pemahaman
              warga belajar usai membaca materi.
            </p>
          </div>
          <Button
            variant="outline"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pt-6 border-t border-slate-100">
        ...
        <Card className="p-6 border-slate-200/60 bg-white shadow-sm rounded-2xl lg:col-span-1 h-fit space-y-6">
          <div>
            <h3 className="text-lg font-black text-[#280f91] mb-1">
              Pengaturan Tugas Sesi {sessionNumber}
            </h3>
            <p className="text-xs font-semibold text-slate-500">
              Unggah soal dan atur batas waktu tugas.
            </p>
          </div>

          <div
            onClick={() => !saving && tugasUploadRef.current?.click()}
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setDueDate(
                      `${e.target.value}T${dueDate && dueDate.includes("T") ? dueDate.split("T")[1].substring(0, 5) : "00:00"}`,
                    )
                  }
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDueDate(
                      `${dueDate ? dueDate.split("T")[0] : new Date().toISOString().split("T")[0]}T${e.target.value}`,
                    )
                  }
                  className="w-full text-sm border-slate-200 rounded-lg p-2 bg-slate-50"
                >
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  type="date"
                  value={cutoffDate ? cutoffDate.split("T")[0] : ""}
                  onChange={(e) =>
                    setCutoffDate(
                      `${e.target.value}T${cutoffDate && cutoffDate.includes("T") ? cutoffDate.split("T")[1].substring(0, 5) : "00:00"}`,
                    )
                  }
                  className="w-full text-sm border-slate-200 rounded-lg p-2 bg-slate-50"
                />
                <select
                  ...
                  onChange={(e) =>
                    setCutoffDate(
                      `${cutoffDate ? cutoffDate.split("T")[0] : new Date().toISOString().split("T")[0]}T${e.target.value}`,
                    )
                  }
                  className="w-full text-sm border-slate-200 rounded-lg p-2 bg-slate-50"
                >
          ...
          <Button
            onClick={handleSaveTugasPengaturan}
            disabled={saving}
            className="w-full bg-[#ff6105] hover:bg-[#e05200] text-white font-bold"
          >
            Simpan Pengaturan
          ...
          <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
              <h3 className="text-lg font-black text-[#280f91]">
                Penilaian Tugas Warga Belajar Sesi {sessionNumber}
              </h3>
            </div>
            <Button
              onClick={() => handleDownloadAllZip(sessionId!)}
              disabled={!sessionId}
              size="sm"
              variant="outline"
              className="border-[#280f91] text-[#280f91] hover:bg-[#280f91] hover:text-white font-bold h-9"
            >
              <DownloadCloud className="w-4 h-4 mr-2" /> Download Semua (.ZIP)
            </Button>
          </div>

          <div className="overflow-x-auto p-4 max-h-[400px] overflow-y-auto">
              ...
              <tbody className="divide-y divide-slate-50">
                {submissions.length === 0 ? (
                  <tr className="hover:bg-slate-50/50 transition-colors">
                    <td
                      colSpan={5}
                      className="py-8 px-4 text-center font-bold text-slate-500 italic"
                    >
                  ...
                  submissions.map((sub) => {
                    const currentEdit = gradeEdits[sub.id] || {
                      grade: sub.grade ?? "",
                    ...
                    return (
                      ...
                      >
                        <td className="py-3 px-4 font-bold text-slate-700">
                          {sub.studentName || `Siswa ID: ${sub.studentId}`}
                        </td>
      ...
      {showAttendance && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl p-6 relative">
            <h3 className="text-xl font-black text-[#280f91] mb-4">
              Laporan Kehadiran Sesi {sessionNumber}
            </h3>
      ...
      {showForum && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl w-full max-w-4xl shadow-2xl p-6 relative flex flex-col max-h-[90vh]">
            <h3 className="text-xl font-black text-[#280f91] mb-4 border-b border-slate-100 pb-2">
              Forum Diskusi Sesi {sessionNumber}
            </h3>
              ...
              {forumPosts.length === 0 ? (
                <div className="p-8 text-center text-slate-500 border border-slate-100 rounded-xl bg-slate-50/50">
                  ...
                  <p className="font-medium">
                    Belum ada diskusi pada sesi ini.
                  </p>
                </div>
              ) : (
                forumPosts
                  ...
                  .map((post) => {
                    const isTutorSelf =
                      post.authorId === user?.id && post.authorRole === "tutor";
                    const displayName = isTutorSelf
                      ? "Tutor (Anda)"
                      : post.authorRole === "siswa"
                        ? `Siswa (${post.authorName || "Unknown"})`
                        : post.authorName;
                    const avatarLetter = isTutorSelf
                      ? "T"
                      : (post.authorName || "?").charAt(0).toUpperCase();
                    const avatarColorClass = isTutorSelf
                      ? "bg-[#280f91] text-white"
                              ...
                              <div className="mt-2 space-y-2">
                                ...
                                <div className="flex gap-2 justify-end">
                                  ...
                                  <Button
                                    ...
                                    size="sm"
                                    className="bg-[#280f91] hover:bg-[#3a1bca] text-white h-8"
                                  >
                                    Simpan
                                  </Button>
                                </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex justify-end">
                  <Button
                    onClick={() => {
                      setActiveReplyId(null);
                      submitReply(undefined, topicText);
                    }}
                    className="bg-[#280f91] hover:bg-[#3a1bca] text-white px-6 font-bold"
                  >
                    Kirim Topik
                  </Button>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl w-full max-w-3xl shadow-2xl relative flex flex-col max-h-[90vh] overflow-hidden">
            <h3 className="text-xl font-black text-[#280f91] mb-4 border-b border-slate-100 pb-4 pt-6 px-6 flex items-center gap-2 shrink-0 bg-white z-10 sticky top-0">
              <PenTool className="w-5 h-5 text-purple-600" /> Kelola Soal
              Latihan Sesi {sessionNumber}
            </h3>
              ...
              {isLatihanLoading ? (
                ...
                <div className="p-8 text-center text-slate-500 border border-slate-100 rounded-xl bg-slate-50/50 flex flex-col items-center justify-center">
                  ...
                  <p className="text-sm mt-1 mb-4">
                    Belum ada soal latihan untuk sesi ini atau semua soal telah
                    dihapus. Jangan lupa klik "Simpan Soal Latihan" jika Anda
                    ingin menghapusnya secara permanen dari sistem.
                  </p>
                </div>
              ) : (
                <div className="space-y-6">
                  {questions.map((q, qIdx) => (
                    <div
                      key={qIdx}
                      className="p-5 border border-slate-200 rounded-2xl space-y-4 relative bg-white shadow-sm"
                    >
                      <Button
                        variant="ghost"
                        onClick={() =>
                      ...
                      <div className="space-y-3">
                        {q.options.map((opt, optIdx) => (
                          <div key={optIdx} className="flex items-center gap-3">
                            ...
                            />
                            <span className="text-sm font-black text-slate-400 w-6">
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex flex-col sm:flex-row gap-3 justify-end shrink-0 z-10 sticky bottom-0 rounded-b-3xl">
              <Button
                ...
                className="border-purple-200 text-purple-700 font-bold hover:bg-purple-50 rounded-xl h-11"
              >
                + Tambah Soal
              </Button>
              <Button
