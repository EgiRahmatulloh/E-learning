...
export default function PendahuluanTab({ activeTab, user }: Props) {
  ...
  const [ratFile, setRatFile] = useState<{ title: string; url: string } | null>(
    null,
  );
  const [tertibFile, setTertibFile] = useState<{
    title: string;
    url: string;
  } | null>(null);
    ...
    async function fetchData() {
      ...
      try {
        ...
        const sessionRes = await fetch(
          `/api/elearning/session?courseId=${courseData.data.id}&sessionNumber=0`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
        ...
        toast.error("Gagal memuat data pendahuluan", {
  ...
  const handleFileChange = async (
    ...
    try {
      ...
      toast.success(`File ${file.name} berhasil diunggah dan disimpan!`, {
        id: toastId,
      });
    } catch (error: any) {
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium mt-1">
              Pesan sapaan, penjelasan singkat mengenai CP, dan instruksi
              perkenalan untuk warga belajar.
            </p>
          </div>
          <Button
        ...
        <Card className="p-6 border-slate-200/60 bg-white shadow-sm rounded-2xl">
          ...
          >
            <input
              ...
              ref={ratInputRef}
              accept=".pdf,.doc,.docx"
              onChange={(e) => handleFileChange(e, "rat")}
            />
            {ratFile ? (
              <>
                ...
                <p className="text-xs text-emerald-600 mt-1">
                  Berhasil diunggah. Klik untuk ganti file.
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Maksimal: 10MB (.PDF, .DOCX)
                </p>
                <Button
                  size="sm"
                  className="mt-4 bg-[#280f91] hover:bg-[#ff6105] text-white"
                >
                  Pilih File RAT
                </Button>
              </>
        ...
        <Card className="p-6 border-slate-200/60 bg-white shadow-sm rounded-2xl">
          ...
          >
            <input
              ...
              ref={tertibInputRef}
              accept=".pdf,.doc,.docx"
              onChange={(e) => handleFileChange(e, "tertib")}
            />
            {tertibFile ? (
              <>
                <CheckCircle className="w-10 h-10 text-emerald-500 mx-auto mb-3" />
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 font-medium">
              Lihat dan balas salam perkenalan dari warga belajar.
            </p>
          </div>
        </div>
          ...
          {forumPosts.length === 0 ? (
            <div className="p-8 text-center text-slate-500 border border-slate-100 rounded-xl bg-slate-50/50">
              ...
              <p className="font-medium">
                Belum ada warga belajar yang memperkenalkan diri.
              </p>
            </div>
          ) : (
                          ...
                          {editingMessageId === post.id ? (
                            <div className="mt-2 space-y-2">
                              <textarea
                                ...
                                className="w-full min-h-[80px] border border-slate-200 rounded-xl p-3 text-sm focus:outline-none focus:border-[#280f91] bg-slate-50"
                              />
                              <div className="flex gap-2 justify-end">
                                <Button
                                  onClick={() => setEditingMessageId(null)}
          ...
          <div className="mt-4 pt-4 border-t border-slate-100">
            ...
            <div className="flex gap-2">
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && submitReply(undefined, topicText)
                }
